	subroutine rvmat(amaxrf,amaxr,amaxr05,amaxr95)
c-----
...
c       dens=xdens
 	...
 	call gcmdln('rvcal',mdfile,jout,dofilt,dofourier,
...
c-----
         write(*,*)fn,dist,rref,amaxrf,amaxr,amaxr05,amaxr95,
...
C		write(6,*)tstar,fz,eta
	...
	subroutine comvalues(pname,mdfile,jout,dofilt,dofourier,
     1		doftbar,instyp,xmommg,isvert,dist,fn,damp,
     1		rref,dt,niter,kseed,str)
	character pname*(*)
	character mdfile*(*)
...
c-----
	...
	nmarg = mnmarg()
c	if(nmarg .eq. 0)call usage(pname)
	i = 0
   11	continue
	i = i + 1
...
c-----
	...
	ls = lgstr(pname)
cwrite(LER,*)'Propagation model description: ',pname(1:ls),' [options]'
	write(LER,*)' description is given at right in [] s '
	write(LER,*)' '
cwrite(LER,*)' The KEYWORD defines the next parameter(s). If KEYWORD'
