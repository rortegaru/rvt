	program fscal
c-----
c	Output Fourier Spectra Prediction
c-----
c	common blocks
c-----
	parameter(NDMAX=100)
...
c-----
	call gcmdln('fscal',mdfile,jout,dofilt,dofourier,
...
c-----
	call output(fn,dist,rref,amaxrf,amaxr,xmommg,jout,
     1		str,damp,dofilt)

	end

	subroutine output(fn,dist,rref,amaxrf,amaxr,xmommg,jout,
     1		str,damp,dofilt)
c-----
...
c-----
	real fn, dist, rref, amaxrf, amaxr, xmommg, damp
	integer jout
	logical dofilt
	character str*(*)
	character ostr*4
...
C		write(6,*)tstar,fz,eta
		...
		write(6,'(2f10.3,e11.3,1x,a4,1x,2f10.2,l10)')
     1			dist,xmommg,amaxr,ostr,fn,damp,dofilt
	endif
	return
	end
...
c-----
	...
	write(LER,*)
     1	'  -FBAR  (default .false.) Smooth spectra for -B'
	write(LER,*)' '
